...
int main(int argc, char* argv[]) {
    ...
    // Setup
    const char* names[] = { "Doma", "Puma", "Wilamuda", "Ids", "Fukxim", "Naku", "Zai", "Comet"};
    reb_add_fmt(r, "m", 1.0);   // Doma
    reb_add_fmt(r, "m a e inc Omega", 9.54e-4, 1.0, 0.1, 0.0, 0.0);    // Puma
    reb_add_fmt(r, "m a e inc Omega", 3.64e-7, 1.1, 0.1, 0.0, 0.524);    // Wilamuda
    reb_add_fmt(r, "m a e inc Omega", 9.92e-7, 1.2, 0.1, 0.0, 1.047);    // Ids
    reb_add_fmt(r, "m a e inc Omega", 3.84e-7, 1.3, 0.446, 0.0, 0.0);  // Fukxim
    reb_add_fmt(r, "m a e inc Omega", 8.76e-8, 1.4, 0.446, 0.0, 1.047);  // Naku
    reb_add_fmt(r, "m a e inc Omega", 3.20e-6, 1.5, 0.446, 0.0, 0.0);  // Zai
    reb_add_fmt(r, "m a e inc Omega", 1.6e-9, 1.8, 0.746, 0.0, 0.0);   // Comet
    // TODO: Pending adding main belt astroids in next revision
    printf("### Project REBOUND: 20230923 Doma System Rev2\n");
    printf("# Changelog:\n");
    printf("# Rev 1: Preliminary setup.\n");
    printf("# Rev 2: Tests `inc` parameter.\n");
    printf("# Rev 3: Preliminary setup of complete Doma system planets (parameters are not calibrated).\n");
